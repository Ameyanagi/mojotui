...
def test_latest_message_coalesces_by_key() raises:
    ...
    assert_true(queue.enqueue_latest("resize", Message(80)) == EnqueueResult.ACCEPTED)
    assert_true(queue.enqueue_latest("tick", Message(1)) == EnqueueResult.ACCEPTED)
    assert_true(queue.enqueue_latest("resize", Message(120)) == EnqueueResult.COALESCED)
...
def test_lossless_message_evicts_coalescible_before_backpressure() raises:
    ...
    assert_true(queue.enqueue_lossless(Message(3)) == EnqueueResult.ACCEPTED)
...
def test_full_lossless_queue_reports_backpressure_without_loss() raises:
    ...
    _ = queue.enqueue_lossless(Message(2))
    assert_true(queue.enqueue_lossless(Message(3)) == EnqueueResult.BACKPRESSURE)
    assert_equal(len(queue), 2)
    assert_equal(queue.dequeue().value().value, 1)
    assert_equal(queue.dequeue().value().value, 2)
...
def test_new_coalescible_message_reports_drop_at_capacity() raises:
    ...
    _ = queue.enqueue_lossless(Message(1))
    assert_true(
        queue.enqueue_latest("resize", Message(2)) == EnqueueResult.DROPPED_COALESCIBLE
    )
